...
const std::string kArtifact = kRoot + "/models/prism-no-0.2.5";
...
TEST_F(TaggerTest, ExposesArtifactMetadata)
{
    ...
    EXPECT_EQ(tagger.artifact().version(), "0.2.5");
...
TEST_F(TaggerTest, CAbiExposesTheFullResultSurface)
{
    ...
    EXPECT_STREQ(prism_tagger_artifact_version(tagger), "0.2.5");
...
{
    const auto artifact = kRoot + "/models/prism-no-0.2.5-fast";
