...
Per-artifact quality and runtime benchmarks (sizes, the frozen
test-split comparison, fast-versus-fp32 quality, and the cross-binding
chapter runtimes) live in one file per released artifact under
[docs/benchmarks/](benchmarks/), for example
[prism-no-0.2.2](benchmarks/prism-no-0.2.2.md). This file records the
model-development history: training, ablations, and selection gates.
