...
def _english_ud_morphology_policy_steps(
    *,
    language_tag: str,
) -> tuple[UniversalFeaturesPolicyStep, ...]:
    # English UD carries standard features with no canonical->
    # convention remapping (unlike Norwegian common-gender / Nynorsk
    # steps), so the policy is always empty.
    del language_tag
    return ()
...
def parse_evaluation_arguments(
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate an English student baseline.",
    )
    parser.add_argument(
        "--language-tag",
        choices=("en",),
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("runs/en-student-baseline/best.pt"),
    ...
    parser.add_argument(
        "--analysis",
        type=Path,
        default=Path("runs/en-student-baseline/development-analysis-logit-zero.json"),
        dest="analysis_path",
    )
    parser.add_argument(
        "--device",
...
def main() -> None:
    ...
    if not isinstance(
        ...
        str,
    ) or not english_model_supports_language_tag(
        checkpoint_language_tag,
    ...
    schema = build_english_schema(schema_training_tokens)
    ...
    development_corpus = encode_english_sentences(
        development_tokens,
        schema=schema,
    )
    ...
    universal_features_policy_steps = (
        ...
        else _english_ud_morphology_policy_steps(language_tag=profile.language_tag)
    )
    ...
    metrics = evaluate_supervised_token_task_epoch(
        ...
        universal_dependencies_accumulator=(
            UniversalDependenciesEvaluationAccumulator(
                schema=schema,
                reference_batches=development_reference_batches,
                lemma_decoder=build_english_ud_lemma_decoder(schema_training_tokens),
                universal_features_policy_steps=universal_features_policy_steps,
            )
        ),
...
def _lemma_rule_training_counts(
    *,
    ...
    for raw_sentence in schema_training_tokens:
        ...
        sentence = encode_english_sentence(
