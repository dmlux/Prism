...
def apply_morphology_feature_logit_correction(
    *,
    ...
    """Remove weighted-loss prior shift from one morphology feature."""
    ...
    if weights.ndim != 1 or weights.shape[0] != feature_schema.logit_count:
        raise ValueError(
            "Morphology logit-correction weights must match feature logits."
        )
    if not weights.is_floating_point():
        raise ValueError("Morphology logit-correction weights must be floating point.")
    if not torch.isfinite(weights).all().item() or torch.any(weights <= 0.0).item():
        raise ValueError(
            "Morphology logit-correction weights must be finite and positive."
        )

    resolved_weights = weights.to(
        device=feature_logits.device,
        dtype=feature_logits.dtype,
    )
    return feature_logits - strength * resolved_weights.log()
...
def decode_morphology_feature_logits(
    *,
    feature_logits: torch.Tensor,
    feature_schema: MorphologyFeatureSchema,
) -> torch.Tensor:
    """Decode one categorical or multi-value morphology feature."""
    ...
    return F.one_hot(
        prediction_ids,
        num_classes=len(feature_schema.labels),
    ).to(dtype=torch.bool)
