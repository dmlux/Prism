...
try:
    if is_flash_attn_2_available():
        ...
        logger.warning_once(
            "FlashAttention is not available in this environment; using the portable (eager) attention path. FlashAttention (CUDA-only) would be faster and use less memory, but is not required."
        )
except ImportError:
    flash_attn_varlen_qkvpacked_func, RotaryEmbedding, apply_rotary = None, object, None
    logger.warning_once(
...
class RotaryPositionalEmbeddings(nn.Module):
    def __init__(self, config, theta: int):
        ...
        # PRISM MODIFICATION (vendored change): upstream registers these as
        # persistent=False and recomputes them per load (LTG's HF integration
        # relies on a `reinitialize_non_persistent_buffers` flag). transformers
        # 5.x `from_pretrained` builds on the meta device and does NOT recompute
        # __init__-time non-persistent buffers, leaving them as uninitialised
        # memory (NaN) -> the RoPE output is then NaN on any fresh load. We make
        # them persistent so they are saved and restored on every load path
        # (from_pretrained, tagger backbone load, resume); deterministic and tiny
        # (~256 KB for max_pos 512 x head 64).
        self.register_buffer("cos_matrix", embedding.cos(), persistent=True)
        self.register_buffer("sin_matrix", embedding.sin(), persistent=True)
    ...
    def get_contextualized_embeddings(
        ...
        if flash_attn_varlen_qkvpacked_func is not None:
            if len(attention_mask.size()) != 2:
                raise ValueError("Only a 2-dimensional `attention_mask` is supported for FlashAttention.")
            with torch.no_grad():
