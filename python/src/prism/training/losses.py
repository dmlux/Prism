...
def calculate_morphology_feature_loss(
    *,
    feature_logits: Tensor,
    feature_targets: Tensor,
    feature_schema: MorphologyFeatureSchema,
    token_mask: Tensor,
    feature_weights: Tensor | None = None,
) -> Tensor:
    """Calculate one schema-driven morphology feature loss."""
    ...
    if token_mask.shape != feature_logits.shape[:-1]:
        raise ValueError(
            "Morphology feature token mask must match the logits' token dimensions."
        )
    if token_mask.dtype != torch.bool:
        raise ValueError("Morphology feature token mask must use torch.bool.")
    if feature_weights is not None and feature_weights.shape != (
        feature_schema.logit_count,
    ):
        raise ValueError("Morphology weights must match the feature logit count.")

    valid_targets = feature_targets[token_mask]
