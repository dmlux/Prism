...
| Fact | Norwegian (`prism-no` 0.2.5) | English (`prism-en` 0.1.0) |
| --- | --- | --- |
...
| --- | ---: | --- |
| `prism-no-0.2.5-fast` | ≈ 45 MB | **Norwegian**, recommended. int8, up to 2× faster, quality within 0.014 pp of fp32 |
| `prism-no-0.2.5` | ≈ 94 MB | Norwegian, bit-exact fp32 reference behind the published benchmark |
| `prism-en-0.1.0` | ≈ 69 MB | **English** (`en`), fp32. int8 is a tracked follow-up for the ModernBERT backbone |

```bash
curl -LO https://github.com/dmlux/Prism/releases/download/prism-no-0.2.5/prism-no-0.2.5-fast.tar.gz
tar -xzf prism-no-0.2.5-fast.tar.gz   # unpacks the prism-no-0.2.5-fast/ folder
```
...
`"prism-no-0.2.5-fast"` in the snippets below means *the unpacked
...
let artifactDirectory = Bundle.main.resourceURL!
    .appendingPathComponent("prism-no-0.2.5-fast")
...
prism::tagger::Tagger tagger("prism-no-0.2.5-fast");
...
prism_tagger* tagger = prism_tagger_create("prism-no-0.2.5-fast");
prism_result* result = prism_tagger_tag_text(tagger, "Hun kjøpte tre gamle bøker.");
...
Path artifact = Path.of("prism-no-0.2.5-fast");
